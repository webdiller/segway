...
export default function FeatureThirdTypeOne({customClass, title, text, mediaBreakpoint = 768, mobileBackgroundImage, desktopBackgroundImage, mobileImage, desktopImage, mobileWidth, mobileHeight, desktopWidth, desktopHeight, lastText}) {
  ...
  return (
    <div className={customClass ? `feature-third-type-one feature-third-type-one_${customClass}` : 'feature-third-type-one'}>
      <div className="feature-third-type-one__bg-image-wrapper">
        {mediaQuery ? <Image objectFit="cover" className="feature-third-type-one__bg-image" src={desktopBackgroundImage} alt={title} layout="responsive" /> : <Image objectFit="cover" className="feature-third-type-one__bg-image" src={mobileBackgroundImage} alt={title} layout="responsive" />}
      </div>
